...
app.post("/register/", async (request, response) => {
  ...
  if (dbUser !== undefined) {
    ...
    const addNewUserQuery = `INSERT INTO user (username,password,name,gender)
...
app.get("/user/followers/", authenticateToken, async (request, response) => {
  ...
  `;
  const followers = await database.all(getFollowerQuery);
...
app.get(
  ...
  `;
    ...
    WHERE tweet_id = ${tweet_id};`;
      const likedUserIdObjectList = await database.all(getLikesQuery);
      const likedUserIdsList = likedUserIdObjectList.map((object) => {
        return object.user_id;
      });
      const getLikedUserQuery = `
    SELECT username FROM user
    WHERE user_id IN (${likedUserIdsList});`;
      const likedUserObjectsList = await database.all(getLikedUserQuery);
      const likedUsersList = likedUserObjectsList.map((object) => {
...
app.get("/user/tweets/", authenticateToken, async (request, response) => {
  ...
  const getLikeQuery = `
     ...
     ORDER BY tweet_id;`;
  const likesObjectsList = await database.all(getLikeQuery);
  const getRepliesQuery = `
     SELECT COUNT(reply_id) AS replies FROM reply
     WHERE tweet_id IN (${tweetIdsList}) GROUP BY tweet_id
     ORDER BY tweet_id;`;
